export const roleDB = [
  {
    id: "36029d8a-4086-444a-8d98-2a264e5aac37",
    name: "guest",
    isActive: true
  },
  {
    id: "2186b6a7-517b-4190-9028-5c1a7d37aff0",
    name: "editor",
    isActive: true
  },
  {
    id: "e0aa07dc-4ff2-4e7a-b1db-7c675e1983e9",
    name: "admin",
    isActive: true
  }
];

export const assetDB = [
  {
    affiliate: "Affiliate1",
    archive: false,
    brand: "Aquaflux",
    businessQuestion: {
      description: "What were the sales during the second quarter of this year?",
      idBq: "773353f8-5c22-4d31-a744-9aa8f1b0ce23",
      name: "Business Question"
    },
    category: "Featured",
    chartData: [
      {
        date: "06/01/2024",
        value: 10
      },
      {
        date: "06/02/2024",
        value: 20
      },
      {
        date: "06/03/2024",
        value: 30
      },
      {
        date: "06/04/2024",
        value: 40
      },
      {
        date: "06/05/2024",
        value: 50
      }
    ],
    country: "Grenada",
    date: "06/15/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: true,
    id: "a749b385-5bb4-499d-91a6-9f7229408245",
    name: "KPI1",
    region: "Caribbean",
    shortDescription: "Short description of the item goes nicely here.",
    value: 50
  },
  {
    affiliate: "Affiliate2",
    archive: false,
    brand: "Cirqula",
    category: "Featured",
    country: "Germany",
    date: "06/16/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: true,
    id: "3813f2fa-db0c-43e1-981c-1cf34c147bbb",
    name: "KPI2",
    region: "Central Europe",
    shortDescription: "Short description of the item goes nicely here.",
    value: 80
  },
  {
    affiliate: "Affiliate5",
    archive: false,
    brand: "Nexnova",
    businessQuestion: {
      description: "How do sales during the second quarter of this year compare to the second quarter of last year?",
      idBq: "65d63dfc-770a-4633-b407-a999fed32321",
      name: "Business Question"
    },
    category: "Featured",
    chartData: [
      {
        date: "06/01/2024",
        value: 50
      },
      {
        date: "06/02/2024",
        value: 40
      },
      {
        date: "06/03/2024",
        value: 30
      },
      {
        date: "06/04/2024",
        value: 20
      },
      {
        date: "06/05/2024",
        value: 10
      }
    ],
    country: "Yemen",
    date: "06/17/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: true,
    id: "d9b13448-37be-4a72-8c20-d9291c994612",
    name: "KPI3",
    region: "Western Asia",
    shortDescription: "Short description of the item goes nicely here.",
    value: 30
  },
  {
    affiliate: "Affiliate3",
    archive: false,
    brand: "Blissify",
    category: "Featured",
    country: "Niger",
    date: "06/18/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: true,
    id: "045f054d-cbfb-44bf-9b09-b38e75d1f4b1",
    name: "KPI4",
    region: "West Africa",
    shortDescription: "Short description of the item goes nicely here.",
    value: 100
  },
  {
    archive: false,
    brand: "Ecliptech",
    businessQuestion: {
      description: "Which sales collateral is viewed most often?",
      idBq: "37478026-5c40-4549-b3c9-3ed73a6d726a",
      name: "Business Question"
    },
    category: "Trending",
    country: "Malawi",
    date: "06/19/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: false,
    id: "be6e2612-b444-4060-a2e6-c723b62b8fd4",
    name: "KPI5",
    region: "East Africa",
    shortDescription: "Short description of the item goes nicely here.",
    value: 99
  },
  {
    archive: false,
    brand: "Enerexis",
    category: "Trending",
    country: "Samoa",
    date: "06/20/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: false,
    id: "ef95004d-3145-43ef-8416-5b42da6d178a",
    name: "KPI6",
    region: "Oceania",
    shortDescription: "Short description of the item goes nicely here.",
    value: 20
  },
  {
    archive: false,
    brand: "Vibranta",
    category: "Trending",
    country: "Malta",
    date: "06/21/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: false,
    id: "243070c8-d4d7-4094-93f3-750f44be9d2a",
    name: "KPI7",
    region: "Mediterranean Sea",
    shortDescription: "Short description of the item goes nicely here.",
    value: 80
  },
  {
    affiliate: "Affiliate2",
    archive: false,
    brand: "Optiquest",
    category: "Trending",
    country: "Bulgaria",
    date: "06/22/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: false,
    id: "9dd4b2d5-c819-4f2e-ae92-741fab759da8",
    name: "KPI8",
    region: "Southeastern Europe",
    shortDescription: "Short description of the item goes nicely here.",
    value: 100
  },
  {
    archive: false,
    brand: "Celestria",
    country: "Finland",
    date: "06/23/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: false,
    id: "4511e6f5-1e27-4429-b667-5b5601c259d7",
    name: "KPI9",
    region: "Northern Europe",
    shortDescription: "Short description of the item goes nicely here.",
    value: 50
  },
  {
    archive: false,
    brand: "Nexgenius",
    businessQuestion: {
      description: "Who are the top-performing sales professionals?",
      idBq: "ab517284-e001-4a83-855c-2fdaf9d221c5",
      name: "Business Question"
    },
    country: "South Africa",
    date: "06/24/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: false,
    id: "e738648f-c4b4-446b-885f-d424e8d79ebd",
    name: "KPI10",
    region: "Southern Africa",
    shortDescription: "Short description of the item goes nicely here.",
    value: 60
  },
  {
    archive: true,
    brand: "Krazero",
    country: "Indonesia",
    date: "06/25/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: false,
    id: "3d2dac7a-7f6d-4f84-9a95-c2d4c60b0a69",
    name: "KPI11",
    region: "Southeast Asia",
    shortDescription: "Short description of the item goes nicely here.",
    value: 35
  },
  {
    archive: true,
    brand: "Zenluxe",
    country: "Fiji",
    date: "06/26/2024",
    description: "Those options are already baked in with this model shoot me an email clear blue water but we need distributors to evangelize the new line to local markets.",
    favorite: false,
    id: "5471720c-970e-4708-b7a7-b10bf878121d",
    name: "KPI12",
    region: "South Pacific",
    shortDescription: "Short description of the item goes nicely here.",
    value: 80
  }
];
